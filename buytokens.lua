--[[
    buytokens.lua
    Navigates to "Walter" and buys "Token of Legendary Might",
    which costs 60 x "Token of Might" each.

    Usage:  /lua run buytokens        -- buy as many as you can afford
            /lua run buytokens max    -- same, explicit
            /lua run buytokens 3      -- buy at most 3 (needs 180 tokens)

    Requires: MQ2Nav loaded, with a navmesh for the current zone.

    Walter's window type is auto-detected:
...
-- Config -------------------------------------------------------------------
local NPC_NAME      = 'Walter'
local ITEM_NAME     = 'Token of Legendary Might'
local CURRENCY_NAME = 'Token of Might'
local COST_EACH     = 60          -- currency units per item
...
local function log(msg)  printf('\ag[buytok]\ax %s', msg) end
local function warn(msg) printf('\ay[buytok]\ax %s', msg) end
local function fail(msg) printf('\ar[buytok]\ax %s', msg) end
...
local function openWindow()
    ...
    if not MODE then
        ...
        fail('Run:  /lua run wndscan   then right-click Walter, and send the dump.')
        return false
    end
